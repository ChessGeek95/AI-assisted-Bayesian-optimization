...
def comp_ucb_scores(alpha, beta, xy_quers, z_quers):
    ...
    print(x[:5, :10])
    print(y[:5, :10])
    print(points[:5])#.reshape(-1,7,2))
    print(points[101:106])
    #"""
    
    #ucb_scores = ucb_scores.reshape(-1, n_iters).T
...
n_part = 3
params = [[(i/(n_part+1),j/(n_part+1)) for i in range(1, n_part+1)] for j in range(1, n_part+1)]

x_arms, y_arms = trial_1.n_arms
#alpha, beta = trial_1.user_params
print(trial_1.user_params)
#alpha, beta = (0.1, 0.8)
xy_queries = trial_1.xy_queries
z_queries = trial_1.z_queries
