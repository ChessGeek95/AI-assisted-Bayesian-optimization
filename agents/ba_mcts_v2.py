...
class BAMCTS_v2:
    ...
    def learn(self, n_sim, progress_bar=False):
        ...
        for _ in iterations:
            idx = np.random.choice(self.alpha_u.shape[0])
            alpha_u = self.alpha_u[idx]
            beta_u = self.beta_u[idx]
    ...
    def best_action(self):
        ...
        value_children = [node.cumulative_reward for node in self.root.children.values()]
        mean_value_children = [node.cumulative_reward/node.n_visits for node in self.root.children.values()]
        index_best_action = np.argmax(number_of_visits_children)
        a = list(self.root.children.values())[index_best_action].action

        indx = np.argpartition(number_of_visits_children, -5)[-5:]
        """
        print("actions:   ", indx)
        print("n_visit:   ", np.array(number_of_visits_children)[indx])
        print("value:     ", np.round(value_children,2)[indx])
        print("mean value:", np.round(mean_value_children,2)[indx])
        print("best action: ", a, ' => ', number_of_visits_children[a], 
                                    np.round(value_children[a], 2), 
                                    np.round(mean_value_children[a], 2))
        print("-" * 35)
        """
        return a
